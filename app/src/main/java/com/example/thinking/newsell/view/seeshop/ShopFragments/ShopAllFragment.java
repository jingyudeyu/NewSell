...
public class ShopAllFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        NetWorks.getSidCategory(user.getSid(), new BaseObserver<List<Category>>() {
            @Override
    ...
    /*Adapter_GridLayout*/
        ...
        adapters.add(Adapter_GridLayout);
          /*  GridLayoutHelper gridLayoutHelper2 = new GridLayoutHelper(2);
            gridLayoutHelper2.setItemCount(listItem30.size());
            gridLayoutHelper2.setPadding(10, 10, 10, 10);
            gridLayoutHelper2.setAspectRatio(2);
            gridLayoutHelper.setAutoExpand(false);//是否自动填充空白区域
            gridLayoutHelper2.setBgColor(getResources().getColor(R.color.colorWhite));
            Adapter_GridLayout_0 = new SubAdapter(getContext(), gridLayoutHelper2, this.listItem30.size(), this.listItem30, 3);*/
            ...
            adapters.add(Adapter_GridLayout_2);
              ...
              /*  {
                ...
                public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
                    return new CardHolder(LayoutInflater.from(getContext()).inflate(R.layout.homepage_reit_content, parent, false));
                }

                public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
                  if (holder instanceof CardHolder) {
                        if (position == 0) {
                           // ((CardHolder)holder).itemView.setBackgroundColor(Color.GREEN);
                            ((CardHolder)holder).good_textview.setText(((Commodity) listItem.get(0).get("categorysell")).getProductname());
                            Log.v("没有名字吗",((Commodity) listItem.get(0).get("categorysell")).getProductname());
                            Log.v("几个呢",String.valueOf(listItem.size()));
                        }
                        else {
                            Commodity commodity = (Commodity) listItem.get(position).get("categorysell");
                            ((CardHolder) holder).good_textview.setText(commodity.getProductname());
                            ((CardHolder) holder).money_textview.setText(String.valueOf(commodity.getPrice()));
                            Glide.with(getContext())
                                    .load(commodity.getLogo())
                                    .diskCacheStrategy(DiskCacheStrategy.RESULT)   //仅仅缓存最终的图像
                                    .into(((CardHolder) holder).good_imageview);
                        }
                    }
                }
            };*/
      ...
      /*  {
            ...
            // 在构造函数设置每行的网格个数
            // gridLayoutHelper2.setItemCount(listItem30.size());// 设置布局里Item个数
            rangeGridLayoutHelper.setPadding(20, 20, 20, 20);// 设置LayoutHelper的子元素相对LayoutHelper边缘的距离
            rangeGridLayoutHelper.setMargin(20, 20, 20, 20);// 设置LayoutHelper边缘相对父控件（即RecyclerView）的距离
            gridLayoutHelper.setBgColor(Color.GRAY);// 设置背景颜色
            rangeGridLayoutHelper.setAspectRatio(2);// 设置设置布局内每行布局的宽与高的比
            // gridLayoutHelper特有属性
            // gridLayoutHelper2.setWeights(new float[]{50,50});//设置每行中 每个网格宽度 占 每行总宽度 的比例
            rangeGridLayoutHelper.setVGap(20);// 控制子元素之间的垂直间距
            rangeGridLayoutHelper.setHGap(20);// 控制子元素之间的水平间距
            ...
            rangeStyle.setHGap(5);
            rangeStyle.setVGap(5);
            rangeGridLayoutHelper.addRangeStyle(1, listItem30.size() - 1, rangeStyle);

            Adapter_GridLayout_0 = new SubAdapter(getContext(), rangeGridLayoutHelper, this.listItem30.size(), this.listItem30, 3);
        }*/
