...
import com.hyphenate.easeui.newmore.BaseObserver;
import com.hyphenate.easeui.newmore.NetWorks;
import com.hyphenate.easeui.newmore.Shop;
import com.hyphenate.easeui.newmore.UserBuyer;
...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            NetWorks.knowByPhone(username, new BaseObserver<UserBuyer>() {
                @Override
                public void onHandleSuccess(UserBuyer userBuyer) {
                    if (userBuyer.getType() == 0) {
                     finalHolder.name.setText(userBuyer.getUser().getUsername());
                        Glide.with(getContext()).load(userBuyer.getUser().getHeaderpic()).centerCrop().into(finalHolder.avatar);

                    } else if (userBuyer.getType() == 1) {
                        Glide.with(getContext()).load(userBuyer.getBoss().getPic()).centerCrop().into(finalHolder.avatar);
                        //获取boss店铺的名称
                    ...
                    }else  finalHolder.name.setText(username);
                }
                @Override
                public void onHandleError(int code, String message) {
                }
